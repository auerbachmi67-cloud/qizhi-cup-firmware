...
float pid_update(pid_ctrl_t *pid, float sp, float meas) {
    float e = sp - meas;
    pid->integral += e;
...
float pid_update_hold(pid_ctrl_t *pid, float sp, float meas) {
    float e = sp - meas;
...
void pid_reset(pid_ctrl_t *pid) { pid->integral = 0; pid->prev_error = 0; }
